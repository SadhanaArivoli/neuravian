...
Exposed through the ``neuravian-seed-based-connectivity`` console script and
...
from app.reporting import PUBLICATION_DPI, apply_dark_figure, document_shell, figure_block, footer, info_box, key_value_table, metadata_grid, methods_block, statistics_cards
...
def _write_seed_png(path: Path, img: nib.Nifti1Image, seed_label: str) -> None:
    ...
    display.savefig(str(path), dpi=PUBLICATION_DPI)
    display.close()
...
def _write_html_report(
    ...
    content = document_shell("Seed-Based Connectivity Report", f"Seed: {seed_label}", body, footer_html=footer("Generated by the Neuravian seed-based connectivity pipeline."))
    path.write_text(content, encoding="utf-8")


def run(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run Neuravian seed-based connectivity."
    )
    ...
    print(f"[neuravian] Seed-Based Connectivity using {loaded_atlas.spec.display_name}")
    print(f"[neuravian] Seed ROI: {args.seed_roi} — {seed_label}")
    print(f"[neuravian] fMRIPrep derivatives: {fmriprep_dir}")
    ...
    print(f"[neuravian] Selected BOLD: {selection.bold_path}")
    if selection.confounds_path:
        print(f"[neuravian] Selected confounds: {selection.confounds_path}")
    else:
        print("[neuravian] No confounds file found; extracting raw time series.")
    ...
    print("[neuravian] Extracting seed ROI time series…")
    seed_ts = _extract_seed_timeseries(
        selection.bold_path, loaded_atlas, seed_idx, confounds
    )
    print(f"[neuravian] Seed time series: {seed_ts.shape[0]} volumes")

    print("[neuravian] Computing voxelwise connectivity map…")
    ...
    print(f"[neuravian] Wrote connectivity map: {map_path}")

    _write_seed_png(png_path, z_img, seed_label)
    print(f"[neuravian] Wrote connectivity map PNG: {png_path}")

    _write_seed_timeseries(ts_path, seed_ts, seed_label)
    print(f"[neuravian] Wrote seed time series: {ts_path}")
    ...
    print(f"[neuravian] Wrote report: {html_path}")
    print(f"[neuravian] Completed in {metadata['runtime_seconds']}s")
    return 0
