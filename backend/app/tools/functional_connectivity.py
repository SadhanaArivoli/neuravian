...
``neuravian-functional-connectivity`` console script and launched by
...
def _write_html_report(
    ...
    body = (
        "<p>Atlas-based Pearson correlation matrix generated by Neuravian.</p>"
...
def run(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run Neuravian functional connectivity."
    )
    ...
    print(f"[neuravian] Functional Connectivity using {loaded_atlas.spec.display_name}")
    print(f"[neuravian] fMRIPrep derivatives: {fmriprep_dir}")
    print(f"[neuravian] Confound strategy: {confound_strategy}")
    ...
    if selection.confounds_path:
        ...
        if confound_strategy != "none":
            print(
                f"[neuravian] WARNING: confound strategy '{confound_strategy}' requested "
    ...
    if cs.used:
        print(f"[neuravian] Using {cs.n_regressors} confound regressors: {cs.used}")
    elif confound_strategy != "none":
        print("[neuravian] No confound regressors applied (TSV not found).")
    ...
    print(f"[neuravian] Wrote matrix: {csv_path}")
    print(f"[neuravian] Wrote ROI statistics: {roi_stats_csv_path}")
    print(f"[neuravian] Wrote heatmap: {png_path}")
