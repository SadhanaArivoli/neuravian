#!/bin/bash
# NeuroForge wrapper for FSL FNIRT.
#
...
#
# This wrapper translates NeuroForge flag-based arguments (--input, --ref-preset,
...
#   neuroforge-fnirt --input <brain.nii.gz> \
#                    [--aff-mat <affine.mat>] \
...
if [[ -z "$INPUT" ]]; then
    echo "neuroforge-fnirt: error: --input is required" >&2
    exit 1
fi
...
case "$REF_PRESET" in
    ...
    custom)
        if [[ -z "$REF_FILE" ]]; then
            echo "neuroforge-fnirt: error: --ref-file is required when --ref-preset=custom" >&2
            exit 1
        fi
        REF="$REF_FILE"
        REF_MASK=""
        ;;
    *)
        echo "neuroforge-fnirt: error: unknown --ref-preset '$REF_PRESET'; use mni152_1mm, mni152_2mm, or custom" >&2
        exit 1
        ;;
esac
...
case "$CONFIG_PRESET" in
    ...
    custom)
        if [[ -z "$CONFIG_FILE" ]]; then
            echo "neuroforge-fnirt: error: --config-file is required when --config-preset=custom" >&2
            exit 1
        fi
        CONFIG="$CONFIG_FILE"
        ;;
    *)
        echo "neuroforge-fnirt: error: unknown --config-preset '$CONFIG_PRESET'; use T1_2_MNI152_2mm, T1_2_MNI152_1mm, none, or custom" >&2
        exit 1
        ;;
esac
