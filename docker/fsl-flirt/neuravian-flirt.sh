#!/bin/bash
# Neuravian wrapper for FSL FLIRT.
#
# FLIRT's CLI is: flirt -in <input> -ref <ref> -out <output> -omat <matrix> [options]
# This wrapper translates Neuravian flag-based arguments to FLIRT's convention
# and resolves the reference image from built-in MNI templates or a custom path.
#
# Usage inside the container:
#   neuravian-flirt --input <path.nii.gz> [--ref-preset mni152_2mm] [--ref-file <path>]
...
if [[ -z "$INPUT" ]]; then
    echo "neuravian-flirt: error: --input is required" >&2
    exit 1
fi
...
case "$REF_PRESET" in
    ...
    custom)
        if [[ -z "$REF_FILE" ]]; then
            echo "neuravian-flirt: error: --ref-file is required when --ref-preset=custom" >&2
            exit 1
        fi
        REF="$REF_FILE"
        ;;
    *)
        echo "neuravian-flirt: error: unknown --ref-preset '$REF_PRESET'; use mni152_1mm, mni152_2mm, or custom" >&2
        exit 1
        ;;
esac
