/**
 * Methods generation engine for Neuravian.
 *
...
> = {
  ...
  "import-fmriprep-derivatives": (run) =>
    ...
    `were imported into the Neuravian workspace` +
  ...
  "atlas-roi-extraction": (run) => {
    ...
    return (
      `Atlas-based region-of-interest (ROI) statistics were extracted using a Neuravian ` +
  ...
  "connectome-graph-analysis": (run) => {
    ...
    return (
      `Connectome graph-theoretic analysis was performed using a Neuravian native pipeline ` +
...
/**
 * Generate a Methods draft from recorded run metadata for researcher review.
 ...
 * Adds a preamble about Neuravian and a closing reproducibility note.
 */
export function generateMethodsSection(
  ...
  const preamble =
    `All analyses were performed using Neuravian, an open-source neuroimaging ` +
    `workflow platform (https://github.com/SadhanaArivoli/neuravian), ` +
