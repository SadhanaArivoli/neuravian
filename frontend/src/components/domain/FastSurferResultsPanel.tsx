      ...
      <div className="p-4">
        ...
        <div className="space-y-3">
          {SECTIONS.map((group) => {
            ...
            return <details key={group} open={group !== "Raw Inventory"} className="rounded-lg border border-white/8 bg-white/[0.025]"><summary className="cursor-pointer px-3 py-2 text-xs font-semibold text-slate-300">{group} <span className="font-normal text-slate-600">({groupArtifacts.length})</span></summary><div className="grid gap-px border-t border-white/8 bg-white/5 lg:grid-cols-2">{groupArtifacts.map((artifact) => <article key={artifact.path} className="bg-slate-950/95 px-3 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h4 className="text-xs font-medium text-slate-100">{displayName(artifact)}</h4><p className="mt-0.5 truncate font-mono text-[9px] text-slate-500" title={artifact.path}>{artifact.path}</p><p className="mt-1 text-[10px] text-slate-500">{artifact.format} · {artifact.role.replace(/-/g, " ")} · {bytes(artifact.size)}{artifact.hemisphere ? ` · ${artifact.hemisphere} hemisphere` : ""}</p></div><div className="flex shrink-0 items-start gap-1">{artifact.canView && <OpenWithViewer runId={runId} artifact={artifact} candidates={artifacts} onOpenNeuravian={() => openBuiltIn(artifact)} />}<a href={`/api/runs/${runId}/files/${artifact.path}`} download className="rounded border border-white/10 px-2 py-1 text-[10px] text-slate-300">Download</a></div></div>{artifact.unsupportedReason && <p className="mt-2 text-[9px] text-amber-300/80">{artifact.unsupportedReason}</p>}</article>)}</div></details>;
          })}
