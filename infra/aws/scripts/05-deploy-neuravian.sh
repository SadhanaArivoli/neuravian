...
usage() {
  cat <<'EOF'
Usage: 05-deploy-neuravian.sh --config PATH [--identity-file PATH] [--apply]
...
if [[ "${APPLY}" == "true" ]]; then
  ...
  if [[ -z "${CONFIRMATION}" && -t 0 ]]; then
    read -r -p 'Type DEPLOY LOCAL-ONLY NEURAVIAN: ' CONFIRMATION
  fi
  [[ "${CONFIRMATION}" == "DEPLOY LOCAL-ONLY NEURAVIAN" ]] || die "Exact deployment confirmation was not provided"
fi
...
[REMOTE VM] DRY-RUN: verify checkout ${NEURAVIAN_VM_COMMIT}
...
scp "${SSH_OPTIONS[@]}" "${AWS_INFRA_ROOT}/templates/compose.aws-loopback.yaml" \
  "ubuntu@${PUBLIC_IP}:/tmp/neuravian-compose.aws-loopback.yaml"
ssh "${SSH_OPTIONS[@]}" "ubuntu@${PUBLIC_IP}" 'bash -s' <<'REMOTE'
set -euo pipefail
readonly REPO=/home/ubuntu/neuravian
readonly OVERRIDE=${REPO}/compose.aws-loopback.yaml
[[ "$(git -C "${REPO}" rev-parse HEAD)" == "8b9614c328463c9dfcb5337303cadde447985299" ]]
install -o ubuntu -g ubuntu -m 0644 /tmp/neuravian-compose.aws-loopback.yaml "${OVERRIDE}"
rm -f /tmp/neuravian-compose.aws-loopback.yaml
CONFIG_JSON="$(mktemp)"
...
curl -fsS --max-time 20 http://127.0.0.1:8000/api/pipelines >/tmp/neuravian-pipelines.json
for pipeline in pydeface fmriprep fastsurfer; do
  curl -fsS --max-time 20 "http://127.0.0.1:8000/api/pipelines/${pipeline}/preflight" >/tmp/neuravian-${pipeline}-preflight.json
done
...
python3 - <<'PY'
import json
marker = json.load(open("/var/lib/neuravian/bootstrap-complete.json"))
assert marker["scientific_pipelines_run"] is False
for pipeline in ("pydeface", "fmriprep", "fastsurfer"):
    json.load(open(f"/tmp/neuravian-{pipeline}-preflight.json"))
PY
...
PY
info "Neuravian backend/frontend deployed on VM loopback only"
